...
class Page3_1_1 extends StatelessWidget {
  ...
  static List<Widget> list_material = <Widget>[
    tileMake('AI核心', 'AI Core', '用於部署掛機機器人升級，同時也是玩家之間最主要的交易貨幣'),
    tileMake('AI核心集群', 'AI Core Cluster', '由1000個AI核心組成，需要時可分解使用'),
    tileMake('醫療科技零件', 'Medical Tech Parts', '可用於製作醫療物品、壓碎'),
    tileMake('醫療科技集群', 'Medical Tech Cluster', '由1000個醫療科技零件組成，需要時可分解使用'),
    tileMake('彈藥零件', 'Ammunition tech parts', '可用於製作能量電池、壓碎'),
    tileMake('軍用彈藥零件', 'Military ammunition tech parts', '可用於製作反物質電池、壓碎'),
    tileMake('科技碎片', 'Tech Scrap', '可用於製作背包、開箱子、提升分子等級'),
    tileMake('科技集群', 'Tech Scrap Cluster', '由1000個科技碎片組成，需要時可分解使用'),
    tileMake('雜湊處理器', 'Hash Processor', '用於挖礦獲得btc、製作AI'),
    ...
    tileMake('已加密容器類', '已加密容器(locked container)\n稀有已加密容器(locked rare container)\n傳奇已加密容器(locked legendary container)', '可用科碎在終端機解鎖，內含隨機裝備或其他物品'),
    tileMake('稀有裝備箱', 'Rare Equipment Crate', '每天可以在荒坂貨幣換匯領取\n可用科碎在終端機解鎖，內含隨機裝備'),
    tileMake('補給箱[公會]', 'Supply Crate [Gang]', '用於收集公會資源、佔領'),
